...
void solve(int tc){
    ...
    for(auto &x:b)cin>>x;
    int prev=-1,ans=0;
    umap<int,int> val; // val: cnt
    set<ii> cnt; // {cnt, val} so it is sorted
    int bptr=0;
    n++;
    a.pb(1e14);
    b.pb(n);
    sort(all(b));
    FOR(1,a.size())a[i]+=a[i-1];
    FOR(0,n){
        if(bptr<b.size() and b[bptr]==i+1){
            if(cnt.size()){
                if(prev==-1)ans+=val[0];
                else ans+=(*--cnt.end()).F;
            }
            // for(auto &x:cnt)show2(x.F,x.S);
            cnt.clear();
            val.clear();
            bptr++;
            prev=0;
        }
        cnt.erase({val[a[i]],a[i]});
        val[a[i]]++;
        cnt.insert({val[a[i]],a[i]});
    }
    cout<<ans;
