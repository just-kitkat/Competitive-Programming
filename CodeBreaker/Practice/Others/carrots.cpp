...
ll n=0, m=0, k=0, q=0;
int calc(int x, int cuts){
    // split x into cuts
    int remainder=x%cuts;
    int seg=x/cuts;
    int total=seg*seg*(cuts-remainder) + (seg+1)*(seg+1)*remainder;
    return total;
}
int save(int x, int cuts){
    return calc(x,cuts-1)-calc(x,cuts);
}
void solve(int tc){
    cin>>n>>k;
    vi a(n);
    int ans=0;
    priority_queue<vi>pq; // {save, x, cuts}
    for(auto &x:a){
        cin>>x;
        ans+=x*x;
        pq.push({save(x,2),x,1});
    }
    k-=n;
    while(k--){
        vi x=pq.top(); pq.pop();
        ans-=x[0];
        pq.push({save(x[1],x[2]+2),x[1],x[2]+1});
    }
    cout<<ans;
