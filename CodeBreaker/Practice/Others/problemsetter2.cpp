...
void solve(int tc){
    ...
    node3 *st = new node3(0,1000005);
    sort(all(contests),cmp);
    sort(all(problems));
    umap<int,priority_queue<int,vi,greater<vi>>>pmm;
    for(auto &x:problems)pmm[x.F].push(x.S);
    for(auto &x:pmm){
        st->set(x.F,x.F,-x.S.top());
        x.S.pop();
    }
    int ans=0;
    for(auto &x:contests){
        int r=-st->range_min(x.F,1000001) + x.S;
        if(r>0){
            ans+=r;
            st->set()
        }
