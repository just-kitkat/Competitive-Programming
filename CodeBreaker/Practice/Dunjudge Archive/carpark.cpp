...
void solve(int tc){
    ...
    FOR(start,n){
        ...
        if(!x)curr_sa++;
            ...
            if(curr_sa)subarrays.pb(curr_sa);
            curr_sa=0;
        }
    }
    sort(all(subarrays),greater<>());
    int ans=0;
    int aa=min((int)subarrays.size(), s-1);
    //for(auto x:subarrays)cout<<x<<' ';cout<<el;
    FOR(0,aa)ans+=subarrays[i];
    //cout<<start<<' '<<end<<' '<<ans<<el;
    cout<<max(0,n-ans-start-end)<<el;
}
/*
10 100
1 0 1 1 1 1 0 0 1 0
