...
umap<int,umap<int,vi>> adj;
vi vis;

void dijkstra(int s, vector<int> & dist, vector<int> & p, int c) {
    int n = adj.size()+5;
    dist.assign(n, (int)2e18);
    //p.assign(n, -1);
    //vis.assign(n, false);
...
void solve(int tc){
    ...
    // show_vec(dC);
    int ans=INF;
    FOR(0,n)ans=min(ans,dA[i]+dB[i]+dC[i]);
    cout<<ans;
}
