...
void solve(int tc){
    int n;
    cin>>n;
    vector<vi> a(n,vi(2,1));
    FOR(0,n)cin>>a[i][0]>>a[i][1];
    sort(all(a));
    int cur = 0;// Author: JustKitkat
// Status: WIP
...
void solve(int tc){
    ...
    while(1){
        while(i<a.size()){
            if(a[i][0]>cur){if(pq.empty())f=1; break;}
            pq.push(a[i][1]);
            // cout<<"pq push: "<<a[i][1]<<endl;
            i++;
        }
        while(!pq.empty()){
            // cout<<"process: "<<pq.top()<<endl;
            int x=pq.top();pq.pop();
            if(x>=cur){
                cur++;
                break;
        ...
        // cout<<"check: "<<i<<' '<<a.size()<<' '<<cur<<endl;
        if(f||i>=a.size()&&pq.empty())break;
    }
    cout<<cur<<el;
