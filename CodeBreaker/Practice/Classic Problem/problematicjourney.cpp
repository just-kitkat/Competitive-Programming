...
void solve(int tc){
    ...
    vi weights;
    for(auto &x:edges){cin>>x[2]>>x[1]>>x[0];weights.pb(x[0]);}
    sort(all(weights));
    sort(all(edges));
    weights.pb((int)1e9+5);
    discretize(weights);
    m=weights.size();
    int lo=0,hi=m;
    vii adj[n+5];
    ...
    while(lo<hi){
        int mid=weights[(lo+hi)/2];// rnd()%(hi-lo)+lo;
        int ind=(lo+hi)/2;
        queue<ii>qq;
        qq.push({1,0});
        vi vis(n+5);
        while(qq.size()){
            int u=qq.front().F, step=qq.front().S; qq.pop();
            if(vis[u])continue;
            vis[u]=step;
            if(u==n)break;
            for(auto &x:adj[u])if(!vis[x.F] and x.S<=mid)qq.push({x.F,step+1});
        }
        if(vis[n]>k or vis[n]==0)lo=ind+1;
        else hi=ind;
        // show(lo);
    }
    if(lo==m)cout<<-1;else
