...
void solve(int tc){
    ...
    FOR(0,n){
        int x,y;cin>>x>>y;
        w[i]=x,v[i]=y,a=max(a,x);
    }
    vector<pair<bool,ll>> possible(W,pair<bool,int>({0,0}));
    possible[0] = {true,0};
    ll ans=0;
    for (int k = 0; k < n; k++) {
        for (int x = W; x >= 0; x--) {
            if (possible[x].F&&x+w[k]<=W) {
                possible[x+w[k]] = {true,max(possible[x+w[k]].S,possible[x].S+v[k])};
                ans=max(ans,possible[x].S+v[k]);
            }
        }
    }
    cout<<ans<<el;
