...
bool cmp(ti l, ti r){return get<1>(l)<get<1>(r);}
void solve(int tc){
    cin>>n>>k;
    vi a(n);
    for(auto &x:a)cin>>x;
    ...
    for(auto &x:ranges){
        ...
        get<0>(x)=lower_bound(all(a),get<0>(x))-a.begin();
        get<1>(x)=upper_bound(all(a),get<1>(x))-a.begin()-1;
    }
    sort(all(ranges),cmp);
    set<int>ss;
    int cur=0;
    // discretize
    FOR(0,n)a[i]=i;
    node3 *st=new node3(0,1'000'005);
    int total=0;
    for(auto &x:ranges){
        auto &[l,r,t]=x;
        while(cur<=r)ss.insert(cur),cur++;
        int kept=st->range_sum(l,r);
        int keep=t-kept;
        while(keep-->0){
            int t=*--ss.end();
            ss.erase(--ss.end());
    ...
    cout<<n-st->range_sum(0,1000005)<<el;
