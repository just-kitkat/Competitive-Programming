...
ll n=0, m=0, k=0, q=0;
umap<int,vii>adjSmaller, adjLarger;
int vis[1001];
int dfs(int v, int u, umap<int,vii> &adj, bool neww,int mid){
    if(neww) fill(vis,vis+1001,0);
    int res=1;
    vis[v]=1;
    for(auto &x:adj[v]) if(!vis[x.F] and x.S<=mid) res+=dfs(x.F,v,adj,0,mid);
    return res;
}
...
void solve(int tc){
    ...
    FOR(0,n){
        ...
        while(lo<hi){
            int mid=(lo+hi)/2;
            if(dfs(i,-1,adjSmaller,1,mid) + dfs(i,-1,adjLarger,1,mid) - 1 == n){
                hi=mid;
            }else lo=mid+1;
        }
        if(lo!=m)ans[i]=lo+1;
