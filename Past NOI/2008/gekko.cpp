...
void solve(int tc){
    int n,x;
    cin>>n>>x;
    vector<vi> a(n,vector<int>(x));
    int ans=0;
    FOR(0,n)JFOR(0,x)cin>>a[i][j];
    if(n==1){
        FOR(0,x)ans=max(ans,a[0][i]);
        cout<<ans<<el;
        return;
    }
    FOR(1,n)JFOR(0,x){
        if(j==0)a[i][j]=+max(a[i-1][j],a[i-1][j+1]);
        else if(j==x-1)a[i][j]+=max(a[i-1][j],a[i-1][j-1]);
        else a[i][j]+=max({a[i-1][j-1],a[i-1][j],a[i-1][j+1]});
        if(i==n-1)ans=max(ans,a[i][j]);
    }
    cout<<ans<<el;
