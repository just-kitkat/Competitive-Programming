...
void solve(int tc){
    n=readInt();
    sufmin[n]=1e9+2;
    FOR(0,n)a[i].F=readInt(),a[i].S=readInt();
    DFOR(n-1,0)sufmin[i]=min(sufmin[i+1],a[i].F-a[i].S);
    int ans=0,prefmax=0;
    FOR(0,n){
        ans+=(i==n-1 or a[i].F-a[i].S<sufmin[i+1]) and (i==0 or a[i].F+a[i].S>prefmax);
        prefmax=max(prefmax,a[i].F+a[i].S);
    }
    cout<<ans;
