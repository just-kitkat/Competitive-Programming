...
ll n=0, m=0, k=0, q=0;
umap<int,vi>adj;
vi par,depth,edges,ans,a;
void predfs(int v, int u, int d){
    depth[v]=d, par[v]=u;
    for(auto &x:adj[v])if(u!=x)predfs(x,v,d+1);
}
int cancel=0;
...
void solve(int tc){
    ...
    FOR(0,m-1){
        ...
        if(!none)break;
        while(u!=v and none){
            while(u!=v and depth[u]>=depth[v]) edges[u]++, none&=edges[u]<=2, u=par[u];
            while(u!=v and depth[v]>=depth[u]) edges[v]++, none&=edges[v]<=2, v=par[v];
        }
    }
    dfs(a[0],-1);
    FOR(1,n+1)cout<<min(none,ans[i])<<el;
}
