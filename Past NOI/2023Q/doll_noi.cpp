...
void solve(int tc){
    cin>>n;
    auto uf=UnionFind(1000005);
    uset<int> seen;
    int total=0;
    FOR(0,n){
        int x;
        cin>>x;
        if(seen.count(x)){cout<<total<<el;continue;}
        if(x!=1 and seen.count(x-1)){
            total-=uf.sizeOfSet(x-1)/2+(uf.sizeOfSet(x-1)&1);
            uf.unionSet(x,x-1);
        }
        if(seen.count(x+1)){
            total-=uf.sizeOfSet(x+1)/2+(uf.sizeOfSet(x+1)&1);
        ...
        total+=uf.sizeOfSet(x)/2+(uf.sizeOfSet(x)&1);
        seen.insert(x);
        cout<<total<<el;
    }
