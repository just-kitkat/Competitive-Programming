...
void solve(int tc){
    ...
    //l = k * a**x * b**y
    int mx=log10(l)/log10(a) + 10;
    int my=log10(l)/log10(b) + 10;
    set<int> kk;
    vector<ll> powa(mx+1),powb(my+1);
    powa[0]=1;
    FOR(1,mx+1)powa[i]=powa[i-1]*a;
    powb[0]=1;
    FOR(1,my+1)powb[i]=powb[i-1]*b;

    FOR(0,mx+1){
        JFOR(0,my+1){
            ld pk=(ld)l/(powa[i]*powb[j]);
            if(pk<1)break;
            if(floor(pk)==pk)kk.insert(pk);
        }
        if(powa[i]>l)break;
    }
    cout<<kk.size()<<el;
