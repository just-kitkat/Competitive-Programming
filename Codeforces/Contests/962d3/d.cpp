...
ll n=0, m=0, k=0, q=0,x;
void solve(int tc){
    cin>>n>>x;
    int ans=0;
    FOR(1,x+1){
        JFOR(1,x+1){
            if(i+j>=x || i*j+i+j>n)break;
            // show2(i,j);
            int lo=1,hi=x-i-j,mid=1;
            while(lo<hi){
                mid = lo+(hi-lo)/2;
                if(i*j + i*mid + j*mid==n){lo=mid;break;}
                if(i*j + i*mid + j*mid>n)hi=mid-1;
                else lo=mid+1;
                // show3(mid,lo,hi);
            }
            if(i*j+j*lo+i*lo>n)lo--;
            mid=lo;
            // show(mid);
            ans+=mid;
            // ans+=mid*6;
