...
void solve(int tc){
    ...
    string a,b;
    cin>>a>>b;
    int pref[n+1][26]={0}, prefB[n+1][26]={0};
    FOR(0,n){
        JFOR(0,26){
            if(a[i]-'a'==j)
                pref[i+1][a[i]-'a']=pref[i][a[i]-'a'] + 1;
            else
                pref[i+1][j]=pref[i][j];
            if(b[i]-'a'==j)
                prefB[i+1][b[i]-'a']=prefB[i][b[i]-'a'] + 1;
            else
    ...
    FOR(0,q){
        ...
        int ans=0;
        FOR(0,26){
            if(abs(pref[r][i]-pref[l-1][i] - 
            (prefB[r][i]-prefB[l-1][i])));else continue;
            ans+=abs(pref[r][i]-pref[l-1][i] - 
            (prefB[r][i]-prefB[l-1][i]));
            // show3(pref[l-1][i], pref[r][i], i)
            // show3(prefB[l-1][i], prefB[r][i], i)
        }
        cout<<ans/2<<el;
    }
