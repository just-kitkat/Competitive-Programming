...
void solve(int tc){
    ...
    int bInd=0, i=0;
    FOR(0,n){
        if(bInd!=m and a[i]>=b[bInd])bInd++, pref[i+1]=pref[i]+1;
        else pref[i+1]=pref[i];
    }
    bInd=m-1;
    DFOR(n-1,0){
        if(bInd!=-1 and a[i]>=b[bInd])bInd--, suff[i]=suff[i+1]+1;
    ...
    FOR(0,n+1){
        // show2(i,pref[i]);
        if(pref[i] + suff[i] + 1 >= m)ans=min(ans,b[pref[i]]);
    }
    if(pref[0]+suff[0]>=m)ans=0;
