...
void solve(int tc){
    ...
    for(auto &x:b)cin>>x;
    int pos=0;
    for(auto &x:b)if(x!=-1)pos=1;
    if(pos==1){
        int f=0;
        int flag=1;
        FOR(0,n)if(b[i]!=-1){f=a[i]+b[i];break;}
        FOR(0,n){
            if(b[i]==-1 and f-a[i]>k or f-a[i]<0)flag=0;
            if(b[i]!=-1 and a[i]+b[i]!=f)flag=0;
        }
        cout<<flag<<el;
        return;
    }
    ...
    int flag=1;
    FOR(0,n)if(f-a[i]>k or f-a[i]<0)flag=0;
    if(!flag)cout<<0<<el;
    else cout<<k+*min_element(all(a))-*max_element(all(a))+1<<el;
