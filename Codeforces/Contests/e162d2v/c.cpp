...
void solve(int tc){
    ...
    FOR(0,n){
        int x;cin>>x;
        c.pb({c.back()[0]+(x==1),(ll)c.back()[1]+(x!=1)*x,c.back()[2]+(x!=1)});
    }
    FOR(0,q){
        //pref sum{num of 1, num of 2}
        int a,b;cin>>a>>b;
        a--;
        ll num1=c[b][0]-c[a][0], num2=c[b][1]-c[a][1], num3=c[b][2]-c[a][2];
        // cout<<num1<<' '<<num2<<el;
        if(num1+num2 - num3 < num1*2 || a+1==b || num1==b-a)cout<<"NO\n";else cout<<"YES\n";
    }
