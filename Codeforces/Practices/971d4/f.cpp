...
void solve(int tc){
    ...
    vi pref={0};
    for(auto &x:a)pref.pb(pref.back()+x);
    while(q--){
        int l,r;
        cin>>l>>r;
        int leftShift = (l-1)/n, rightShift = (r-1)/n;
        int ans=total*(rightShift-leftShift-1);
        l = (l-1)%n, r = (r-1)%n;
        int newl=(l+leftShift)%n;
        int newr=(r+rightShift)%n;
        show2(newl, newr);
        if(n-l < n-newl) ans+=pref[newl+n-l]-pref[newl];
        else{
            ans+=pref.back()-pref[newl];
            newl-=n-newl;
            ans+=pref[newl];
            show(ans);
        }
        if(r < n-newr) ans+=pref[newr+n-r]-pref[newr];
        else{
            show(ans);
            ans+=pref.back()-pref[newr];
            show(ans);
            newr=r-(n-newr)+1;
            show(newr);
            ans+=pref[newr];
