...
#define show_vec(a) for(auto &x:a)cout<<x<<' ';cout<<endl;
...
void solve(int tc){
    ...
    FOR(0,n){
        ...
        if(a[i]==a[i-1])curhi++,cnt++,tmp.pb(a[i]);
        else{
            if(a[i]!=a[i-1]+1){
                cnt=1, curhi=1;
            ...
            else if(cnt==1 && curhi>1){
                cnt=1, curhi=2;
                if(ans.size()<tmp.size()) ans=tmp;
                tmp={a[i], a[i-1]};
            }
            else cnt=1, curhi++, tmp.pb(a[i]);
        }
    }
    if(ans.size()<tmp.size()) ans=tmp;
    cout<<ans.size()<<el;
    ...
    int c=0;
    for(auto &x:ans){
        if(c)f.pb(x);else f.push_front(x);
        c=!c;
    }
    show_vec(f);
